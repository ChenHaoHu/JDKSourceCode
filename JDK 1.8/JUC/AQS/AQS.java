...
/**
 * Provides a framework(框架) for implementing blocking（阻塞） locks and related
...
public abstract class AbstractQueuedSynchronizer extends AbstractOwnableSynchronizer implements java.io.Serializable {
    ...
    private static boolean shouldParkAfterFailedAcquire(Node pred, Node node) {
        ...
        // 【SIGNAL】属性表示当前驱节点释放的时候，会通知下去，他可以安静的睡觉觉了
        if (ws == Node.SIGNAL)
            /*
        ...
        //CAS 去循环让前驱节点的属性为 【SIGNAL】 目的很明显 看上文
            /*
             * waitStatus must be 0 or PROPAGATE. Indicate【指示】 that we need a signal, but don't
    ...
    /**
     * Acquires in exclusive uninterruptible (独占不中断) mode for thread already in queue. Used
     * by condition（条件） wait methods as well as acquire.
     *
     * @param node the node
    ...
    final boolean acquireQueued(final Node node, int arg) {
        ...
        try {
            ...
            for (;;) {
                ...
                //前驱节点是否为 head （即头节点，拥有锁） 如果是的，就一直尝试获取锁
                if (p == head && tryAcquire(arg)) {
                    //获取到锁，设置自己为头节点
                    setHead(node);
                    //p为刚才那个头节点，设置他的 next 为 null  帮助gc
                    p.next = null; // help GC
                    //成功了
                    failed = false;
                    //获取到了锁 不中断了
                ...
                //https://img-blog.csdnimg.cn/201811051108341.png 图解
                //在前驱结点不是头节点或者为获取锁的情况下
                // 【shouldParkAfterFailedAcquire】 为 让前驱为【SIGNAL】
                // 【parkAndCheckInterrupt】 为   Convenience method to park and then check if interrupted
    ...
    public final void acquire(int arg) {
        /**
         * 通过tryAcquire获取锁， 如果成功获取到锁直接结束此方法, 否则将当前线程插入队列，然后会 终止(selfInterrupt)
